...
const Process = () => {
  ...
  const steps = [{
    ...
    points: ["Inventering", "Skogsbruksplandata", "Skadeinventering", "Avverkningsuppföljning", "Planteringsuppföljning"],
    ...
    points: ["Datum/tid", "Flygrutt", "Flyginformation", "Uppladdning av Forestand-fil"],
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-12 h-12 text-forest-600">
    ...
    title: "Flygning",
    points: [],
    icon: <div className="w-12 h-12">
          <img src="/lovable-uploads/3f25bc0f-c6df-433b-9337-841fca8583ae.png" alt="Drönare" className="w-full h-full object-contain" />
        </div>
  }, {
    ...
    points: [],
    icon: <img 
            src="/lovable-uploads/4200d1ac-5b96-4575-95e6-bd196ff9baf9.png" 
            alt="AI bildanalys" 
            className="w-12 h-12 text-forest-600"
          />
  }, {
  ...
  return <section id="process" className="py-14 bg-forest-50/70">
          ...
          <CardContent className="p-4">
            <div className="flex gap-4 items-center">
              ...
              <div className="flex-1">
                ...
                {steps[activeStep].subtitle && <p className="text-sm text-forest-600 mt-1">{steps[activeStep].subtitle}</p>}
                
                {steps[activeStep].points.length > 0 && <ul className="grid grid-cols-2 gap-x-4 gap-y-1 mt-3">
                    {steps[activeStep].points.map((point, index) => <li key={index} className="flex items-center gap-1.5 text-sm">
            ...
            <div className="flex justify-between mt-4 pt-2 border-t border-forest-100">
              <ForestButton variant="secondary" size="sm" onClick={() => setActiveStep(prev => Math.max(0, prev - 1))} className={cn("flex items-center gap-1.5", activeStep === 0 && "opacity-50 cursor-not-allowed")} disabled={activeStep === 0}>
              ...
              <ForestButton variant="secondary" size="sm" onClick={() => setActiveStep(prev => Math.min(steps.length - 1, prev + 1))} className={cn("flex items-center gap-1.5", activeStep === steps.length - 1 && "opacity-50 cursor-not-allowed")} disabled={activeStep === steps.length - 1}>
                Nästa
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
